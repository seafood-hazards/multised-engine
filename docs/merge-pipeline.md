...
spans ~2003-2025 while the re-hosted copy runs 1999-2024, so ~342 of 1,054 are the
...
**20,239** (mostly ICES-DOME superseded by MUDAB); rule 2 identifies **712** Vannmiljø
rows superseded by Mareano, but because rule 1's year key already catches most of them
within 1%, rule 2 adds only **21** net; **20,260** distinct duplicates in total.
...
data it flags 656 rows (381 high, 275 low, 0.34%). Idempotent and re-runnable.
...
holds **190,771** measurements from the 211,031-row union (20,260 cross-source
