...
| **slim** | ~~Dropped the Mareano year where the cruise states it without a date, so six cruises (2004, three 2021, 2023, 2025) reached refined with `event.year` NULL~~ (fixed 0.6.1: `year = coalesce(start_year, year)`; a NULL year also exempted those rows from merged's rule 1 dedup). Drops extraction (fixed on `feature/efsa-extraction-class`); drops Mareano `lld.comment` and MUDAB `accreditation`; passes 4Demon `fraction_range` through but nothing consumes it; keeps Vannmiljø `activity` as a code with no meaning attached. | high |
