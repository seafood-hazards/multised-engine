...
# 4Demon has only chemistry (targets + Fe/Al; no organic carbon, no grain-size),
# so there is no Annotate step. Units ug/g (micro sign -> u); depths already cm;
# no lab / lod / loq. The already-folded raw columns (corrected_value, basis,
# vflag, limit_flag, range/outlier flags) are dropped by the keep-list.
#
...
slim_path  <- "./data/db/4demon_slim.sqlite"
clean_path <- "./data/db/4demon_clean.sqlite"

DEPTH_TO_CM <- 1    # 4Demon depths are already in cm (0-20 cm)
...
measurement <- standardise_matrix(measurement)  # matrix -> ICES SED* vocab (FS -> SED63, US -> SEDtot)
