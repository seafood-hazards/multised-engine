...
# ── Clean stage, Step 1: Harmonise (MUDAB) ───────────────────────────────────
# Reads the slim DB and writes ./data/db/mudab_clean.sqlite in a uniform format.
# MUDAB uses the ICES vocabulary (symbols, units, gear codes), so the maps are
# mostly identity and depths are already cm. Mirrors the ICES-DOME template. MUDAB
# has an event `time` column (dropped) and no `src_flag`. Value-preserving.
#
...
slim_path  <- "./data/db/mudab_slim.sqlite"
clean_path <- "./data/db/mudab_clean.sqlite"

DEPTH_TO_CM <- 1    # MUDAB depths are already in cm (0.5-200 cm)
...
dataset   <- standardise_dataset(mutate(dataset, country = "Germany")) # uniform cols; MUDAB is German-operated
...
measurement <- standardise_matrix(measurement)  # matrix -> ICES SED* vocab (PK_default -> NULL)
