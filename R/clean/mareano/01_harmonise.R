...
# Mirrors the ICES-DOME template. Mareano-specific: symbols TOC->CORG and case-fold
# (Fe->FE, Al->AL); depths already cm; method limit column is `lld` (renamed to
# `lod`); no matrix / uncertainty. Value-preserving.
#
...
slim_path  <- "./data/db/mareano_slim.sqlite"
clean_path <- "./data/db/mareano_clean.sqlite"

DEPTH_TO_CM <- 1    # Mareano depths are already in cm (0-49 cm)
...
# Mareano's single limit column is `lld` (LOD, no separate LOQ); rename to `lod`.
if ("lld" %in% names(method)) method <- method |> rename(lod = lld)
...
measurement <- standardise_matrix(measurement)  # matrix -> ICES SED* vocab (no-op here; no matrix)
