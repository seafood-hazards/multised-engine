...
export_refined_dataset <- function(db_dir = multised_db_dir(),
  ...
  dict <- tribble(
    ...
    "ef",                      "",           "Enrichment factor: (element/Al) divided by the offshore background (element/Al) for the same element and fraction. EF < 1 means at or below background. Empty on the sieved fractions, where the sieve is the grain-size control and no enrichment factor is defined; and in bulk, empty where aluminium is missing, where the sample is off its fraction's aluminium basis, or where aluminium does not predict that element.",
