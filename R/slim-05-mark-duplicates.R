# ── Slim step 5: mark duplicates ─────────────────────────────────────────────
# Adds `measurement.dup_flag` (duplicate / technical_replicate, NULL = neither),
# grouping on location + date + depth + element + unit + method, so readings of
# the same element from different methods/labs are not treated as replicates of
# each other. A suspicious marker for manual review, not a de-duplication.
#
# Identical for all five sources.

slim_mark_duplicates <- function(source, db_dir = multised_db_dir(),
                                 verbose = TRUE) {
  ...
  add_column_if_missing(con, "measurement", "dup_flag", "TEXT")
  ...
  # ── 4. Verify ──────────────────────────────────────────────────────────────
  out <- dbGetQuery(con, "SELECT COALESCE(dup_flag,'(none)') dup_flag, COUNT(*) n
