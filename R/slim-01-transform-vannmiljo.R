# ── Slim step 1, Vannmiljø ───────────────────────────────────────────────────
# Pilot tables: activity, client, contractor, site, sample_method,
# analysis_method, sample, parameter, sediment, lld.

slim_transform_vannmiljo <- function(con_src) {
  # ── 0. Read source data ────────────────────────────────────────────────────
  ...
  df_base_method <- df_analysis_method %>%
    inner_join(df_base_sediment %>% distinct(analysis_id), by = "analysis_id")
  ...
  df_base_sample <- df_sample %>%
    inner_join(df_base_sediment %>% distinct(sample_id), by = "sample_id")

  df_base_sampling_tool <- df_sample_method %>%
    inner_join(df_base_sample %>% distinct(method_id), by = "method_id")

  df_base_site <- df_site %>%
    inner_join(df_base_sample %>% distinct(site_code), by = "site_code")

  df_base_activity <- df_activity %>%
    inner_join(df_base_sample %>% distinct(activity_id), by = "activity_id")
  ...
  df_ref_sediment <- df_sediment %>%
    inner_join(df_ref_parameter %>% distinct(param_id), by = "param_id") %>%
