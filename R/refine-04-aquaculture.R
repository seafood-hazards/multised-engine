# ── Refine step 4: aquaculture table + site.aqua_id link ─────────────────────
# Imports the Norway aquaculture reference into the refined DB as the
# `aquaculture` table, and adds `aqua_id` to `site`: the id of the NEAREST
# aquaculture site, so the existing `dist_to_aquaculture` (km) now has an explicit
# link to which farm it refers to. Norway-only (country_code = 'NOR'); NULL
# elsewhere.
#
# Reproduces exactly the distance step that set dist_to_aquaculture
# (R/aquaculture/02_site_distance.R): nearest over ALL aquaculture (active or
# closed, a past farm is a historical pressure), WGS84, great-circle km. The
# nearest-feature index gives the aqua_id; the recomputed distance is
...
refine_aquaculture <- function(db_dir = multised_db_dir(), verbose = TRUE) {
  ...
  acon <- multised_con(aqua_db)
  aqua <- as_tibble(dbReadTable(acon, "aquaculture"))
  dbDisconnect(acon)

  con <- multised_con(refined_db)
  ...
  d_km   <- as.numeric(sf::st_distance(nor_sf, aqua_sf[idx, ], by_element = TRUE)) / 1000
  ...
  invisible(list(n_aqua = nrow(aqua), n_linked = sum(!is.na(site_out$aqua_id)),
                 max_diff = max_diff, pct_exact = pct_exact))
}
