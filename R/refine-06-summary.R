# ── Refine step 6: retention / reconciliation summary ────────────────────────
...
# accounted for), and the coverage of the baked derived fields.

refine_summary <- function(db_dir = multised_db_dir(),
                           analysis_dir = multised_analysis_dir(),
                           verbose = TRUE) {
  merged_db  <- merged_db_path(db_dir)
  refined_db <- refined_db_path(db_dir)
  out_dir    <- file.path(analysis_dir, "refine")
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

  TARGETS <- REFINE_TARGETS
  NORMS   <- REFINE_NORMS

  nrows <- function(con, t) {
    if (t %in% dbListTables(con)) dbGetQuery(con, sprintf("SELECT COUNT(*) n FROM %s", t))$n
    else NA_integer_
  }

  mcon <- multised_con(merged_db)
  rcon <- multised_con(refined_db)
  on.exit({dbDisconnect(mcon); dbDisconnect(rcon)}, add = TRUE)
  ...
  # merged_rows is the source-table count (NA for tables new in refined: the
  # measurement split is owned by the reconciliation CSV)
  tables <- tables |>
    mutate(merged_rows  = purrr::map_int(table, function(x) as.integer(nrows(mcon, x))),
           refined_rows = purrr::map_int(table, function(x) as.integer(nrows(rcon, x))),
  ...
  if (verbose) {
    ...
    tables |> select(table, merged_rows, refined_rows, disposition) |>
  ...
  invisible(list(tables = tables, reconciliation = recon, coverage = coverage))
}
