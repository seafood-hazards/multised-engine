...
# ── 0. Read source data ──────────────────────────────────────────────────────
con_src <- dbConnect(RSQLite::SQLite(), "./data/db/pilot_4demon.sqlite")
...
df_dataset <- df_base_project %>%
  group_by(project)  %>%
  summarise(project_id = paste0(project_id, collapse = ", "),) %>%
  ungroup() %>%
...
df_site <- df_base_station %>%
  ...
  summarise(station_id = paste0(station_id, collapse = ", ")) %>%
  ungroup() %>%
...
df_station_to_site <- df_site %>%
  ...
  separate_rows(station_id, sep = ",\\s*") %>%
  mutate(
...
df_slim <- bind_rows(df_base_sediment, df_ref_sediment) %>%
  ...
  separate(depth_range, sep = "-", remove =FALSE, into=c("depth_from", "depth_to")) %>%
  separate(unit, sep = " ", remove =FALSE, into=c("unit2", "basis")) %>%
  mutate(date_orig = as.Date(sample_timestamp, format = "%d/%m/%Y %H:%M"),
         year = year(date_orig) %>% as.integer(),
