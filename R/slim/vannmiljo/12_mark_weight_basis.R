...
# ── 2. Derive from the unit suffix (Vannmiljø encodes basis as 'dw'/'ww') ────
# e.g. 'mg/kg dw', 'g/kg C dw'. Chemistry with no recognisable suffix stays NULL.
...
d  <- m |> mutate(is_chem = category %in% c("target", "reference", "organic"),
                  weight_basis = if_else(is_chem, wb_from_unit(unit), NA_character_))
