...
# MUDAB uses the ICES `qflag` quality flag. All the non-NULL codes present are
# below-limit indicators: '<' less-than, 'D' below detection limit, 'Q' below
# quantification limit. NULL = an ordinary quantified value. Any unexpected
# non-NULL code is reported below so the mapping stays auditable after a rebuild.
below_codes <- c("<", "D", "Q")
