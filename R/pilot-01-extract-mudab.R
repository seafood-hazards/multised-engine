# \u2500\u2500 Pilot step 1, MUDAB \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
# Parses the two MUDAB exports (the sediment measurement CSV and the wider TSV
# of supporting parameter groups) plus the code list, into the ten pilot tables.
#
# MUDAB ships German column headers, code-list descriptions and free-text
# sediment descriptions. All three are lookup or join keys, so the literals are
# reproduced exactly; the non-ASCII characters are written as \uXXXX escapes
# because R CMD check rejects non-ASCII in code strings.
#
# The original script ended by writing df_all.tsv, code_lookup.tsv and
# location.tsv back into data/raw/Mudab. Those were debugging exports, not part
# of the database build, and are deliberately not reproduced here.

pilot_extract_mudab <- function(raw_dir = multised_raw_dir(), verbose = TRUE) {
  data_path <- file.path(raw_dir, "Mudab")
  ...
  for (f in c(data_file, data_others_file, code_file)) {
    if (!file.exists(f)) stop("MUDAB export not found: ", f, call. = FALSE)
  }
  ...
  df_imet <- read_csv(data_file, show_col_types = FALSE) |>
    ...
    mutate(measurement_date = as.character(measurement_date),
           measurement_time = as.character(measurement_time),
           station_latitude = round(station_latitude, 3),
           station_longitude = round(station_longitude, 3))
  ...
  df_others <- read_tsv(data_others_file) |>
    ...
    filter(parameter_group %in% c("B-BIO", "I-MAJ", "I-NUT", "I-RNC", "O-MAJ",
                                  "P-PHY")) %>%
    inner_join(df_imet %>%
                 distinct(station_latitude, station_longitude, year, measurement_date),
               by = c("station_latitude", "station_longitude", "year", "measurement_date"))
  ...
  # ------------------------------
  # Reference material: 542 (I-MET: 475)
  # ------------------------------
  ...
  # ------------------------------
  # Survey: 3,981 (Original I-MET: 3897)
  # ------------------------------
  ...
  # ------------------------------
  # Sediment 167,124 (I-MET: 109,544)
  # ------------------------------
  df_sediment <- df_all %>%
  ...
  if (verbose) {
    cat(sprintf(paste0("mudab parsed: code_lookup %d, station %d, parameter %d, ",
                       "measurement_time %d, analysis_method %d, ",
                       "reference_material %d, survey %d, lod %d, sample %d, ",
                       "sediment %d\n"),
                nrow(df_code_lookup), nrow(df_station), nrow(df_parameter),
                nrow(df_measurement_time), nrow(df_analysis_method),
                nrow(df_reference_material), nrow(df_survey), nrow(df_lod),
                nrow(df_sample), nrow(df_sediment)))
  }

  list(code_lookup        = df_code_lookup,
       station            = df_station,
       parameter          = df_parameter,
       measurement_time   = df_measurement_time,
       analysis_method    = df_analysis_method,
       reference_material = df_reference_material,
       survey             = df_survey,
       lod                = df_lod,
       sample             = df_sample,
