# ── Analysis, merged generation: enrichment ──────────────────────────────
# Converted from R/analysis/enrichment/01_merged_enrichment.R. The body is unchanged; only the
# hardcoded paths and the console output are parameterised.

analysis_merged_enrichment <- function(db_dir = multised_db_dir(),
  ...
  out_dir <- file.path(out_dir, "enrichment")
