...
out_dir <- "data/analysis/outlier_review"
...
plot_category <- function(cat) {
  ...
  p <- ggplot(dd, aes(logv, colour = fraction)) +
    geom_density(na.rm = TRUE) +
    geom_vline(data = thr, aes(xintercept = med_log),
               linetype = "dotted", colour = "grey40") +
    geom_vline(data = thr, aes(xintercept = lo),
               linetype = "dashed", colour = "firebrick") +
    geom_vline(data = thr, aes(xintercept = hi),
               linetype = "dashed", colour = "firebrick") +
    facet_wrap(~ symbol, scales = "free") +
    labs(title = paste0("log10(value_std) by fraction, category: ", cat),
  ...
  ggsave(file.path(out_dir, paste0("outlier_dist_", cat, ".png")),
