# ── Slim step 1, MUDAB ───────────────────────────────────────────────────────
# Pilot tables: code_lookup, station, parameter, measurement_time,
# analysis_method, reference_material, survey, lod, sample, sediment.
#
# The longest of the five transforms: MUDAB needs a third pass ("SED") to pull
# the physical/grain-size parameters that sit on the same sampling occasion as
# the chemistry, matched on the full sampling key rather than a shared id.
#
# The natural (unkeyed) joins are kept exactly as the original script had them.

slim_transform_mudab <- function(con_src) {
  # ── 0. Read source data ────────────────────────────────────────────────────
  ...
  df_dataset <- bind_rows(df_base_station, df_ref_station, df_sed_station) %>%
    ...
    summarise(region = paste0(unique(region), collapse = ", "), .groups = "drop") %>%
  ...
  df_station_to_dataset <- df_dataset %>%
    ...
    tidyr::separate_rows(station_no, sep = ",\\s*") %>%
    mutate(
  ...
  df_site <- bind_rows(df_base_survey, df_ref_survey, df_sed_survey) %>%
    ...
    summarise(survey_id = paste0(survey_id, collapse = ", "), .groups = "drop") %>%
  ...
  df_survey_to_site <- df_site %>%
    ...
    tidyr::separate_rows(survey_id, sep = ",\\s*") %>%
    mutate(
