...
export_efsa_submission <- function(db_dir = multised_db_dir(),
  ...
  dict <- tribble(
    ...
    "pristineLoc",     "Pristine Location",                  "YESNO",    "pristine_ef, the local-background EF<1 verdict. Bulk only: an enrichment factor is defined only where aluminium is the grain-size control, so this is empty on the sieved fractions even though they carry a pristine verdict of their own (built on the sieve and two concentration criteria, see pristine_strict in the flat dataset). Also empty for the elements D1 withholds",
