# ── Slim step 3: categorize ──────────────────────────────────────────────────
# Adds `element.category`, the single source of truth for the measurand class
# that later steps read instead of each redefining its own symbol lists.
#
# Identical for all five sources.

slim_categorize <- function(source, db_dir = multised_db_dir(), verbose = TRUE) {
  ...
  add_column_if_missing(con, "element", "category", "TEXT")
  ...
  # ── 3. Verify ──────────────────────────────────────────────────────────────
  out <- dbGetQuery(con, "SELECT category, COUNT(*) n FROM element
