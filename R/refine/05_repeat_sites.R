...
# ── Refine stage 5: repeat-sampled-location grouping on `site` ────────────────
# Add two columns to `site`, carrying the repeat-sampled-sites idea (see the
# multised-merged "Repeat-Sampled Sites" analysis) as a reusable fact:
#
...
#
# A location sampled in several years is what later analyses will use for temporal /
# downcore work; the THRESHOLD ("repeat if n_years >= k") is a decision left to those
# analyses, so only the count is stored here. Reads/writes multised_refined.sqlite.
...
print(cells |> count(n_years) |> arrange(n_years) |> head(8) |> as.data.frame(), row.names = FALSE)
