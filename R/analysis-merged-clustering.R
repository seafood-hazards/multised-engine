# ── Analysis, merged generation: clustering ──────────────────────────────
# Converted from R/analysis/clustering/01_merged_clustering.R. The body is unchanged; only the
# hardcoded paths and the console output are parameterised.

analysis_merged_clustering <- function(db_dir = multised_db_dir(),
                                       out_dir = multised_analysis_dir(),
                                       verbose = TRUE) {
  require_suggested("cluster", "The clustering analysis")
  ...
  out_dir <- file.path(out_dir, "clustering")
  ...
  cluster_fraction <- function(feats, fraction) {
    ...
    sweep <- map_dfr(K_RANGE, function(k) {
      ...
      sil <- cluster::silhouette(km$cluster[idx], Dsub)
