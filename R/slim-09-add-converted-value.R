# ── Slim step 9: add converted value ─────────────────────────────────────────
# Adds `value_std` + `unit_std`: a standardised value keyed on the measurand
# class (element.category). Chemistry -> mg/kg, grain-size composition -> %. A
# reusable derived value (range check, Fe/Al normalisation, cross-source merge),
# not a flag.
#
# Identical for all five sources.

slim_add_converted_value <- function(source, db_dir = multised_db_dir(),
                                     verbose = TRUE) {
  ...
  add_column_if_missing(con, "measurement", "value_std", "REAL")
  add_column_if_missing(con, "measurement", "unit_std", "TEXT")
  ...
  # ── 5. Verify ──────────────────────────────────────────────────────────────
  out <- dbGetQuery(con, "SELECT COALESCE(unit_std,'(NULL)') unit_std, COUNT(*) n
