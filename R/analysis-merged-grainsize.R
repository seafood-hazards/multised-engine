# ── Analysis, merged generation: grainsize ───────────────────────────────
# Converted from R/analysis/grainsize/01_merged_grainsize.R. The body is unchanged; only the
# hardcoded paths and the console output are parameterised.

analysis_merged_grainsize <- function(db_dir = multised_db_dir(),
  ...
  out_dir <- file.path(out_dir, "grainsize")
