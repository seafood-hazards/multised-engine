# ── Slim step 4: quality control ─────────────────────────────────────────────
# Adds `site.area_flag` (outside_europe) and `measurement.invalid_flag`
# (negative / over_range). NULL = passed. Suspicious markers for review, not
# deletions.
#
# Identical for all five sources.

slim_quality_control <- function(source, db_dir = multised_db_dir(),
                                 verbose = TRUE) {
  ...
  for (tbl in names(flag_cols)) {
    add_column_if_missing(con, tbl, flag_cols[[tbl]], "TEXT")
  }
  ...
  # ── 5. Verify ──────────────────────────────────────────────────────────────
  area <- dbGetQuery(con, "SELECT COALESCE(area_flag,'(pass)') area_flag, COUNT(*) n
                           FROM site GROUP BY area_flag ORDER BY n DESC")
  invalid <- dbGetQuery(con, "SELECT COALESCE(invalid_flag,'(pass)') invalid_flag, COUNT(*) n
  ...
  invisible(list(area_flag = area, invalid_flag = invalid))
}
