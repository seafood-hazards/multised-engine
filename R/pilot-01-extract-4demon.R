# ── Pilot step 1, 4Demon ─────────────────────────────────────────────────────
# Parses the single 4Demon CSV export into the pilot tables: project, station,
# parameter, method, sample, sediment.
#
# 4Demon ships one wide CSV with cryptic column names; the rename table below is
# the documentation of what each source column means, so it is kept in full.

pilot_extract_4demon <- function(raw_dir = multised_raw_dir(), verbose = TRUE) {
  data_path <- file.path(raw_dir, "4Demon")
  data_file <- file.path(data_path,
                         "20200713_1007454DEMON_HeavyMetals_PCBs_in_sediment.csv")
  if (!file.exists(data_file)) {
    stop("4Demon export not found: ", data_file, call. = FALSE)
  }
  ...
  col_rename <- tibble::tribble(
    ...
    "FSE_RANGE",                   "fraction_range_um",           "Grain size fraction analysed in micrometres (e.g. 0-63 = fine fraction <63 um, 0-2000 = bulk sediment <2 mm).",
    ...
    # --- Matrix and method ---
    "PMX_MTX_CODE",                "matrix_code",                 "Sediment matrix code: FS = fine sediment (<63 um fraction), US = unsieved / bulk sediment.",
    "AMD_SEQNO",                   "method_seq_no",               "Sequential analytical method identifier (102 unique methods).",
    "AMD_SEQNO_new",               "method_code",                 "Harmonised analytical method code combining project, technique, and fraction (e.g. Monit3_OES/MS_63 = monitoring period 3, ICP-OES/MS, <63 um fraction).",
    ...
    "VALUE_ORIG",                  "value",                       "Original measured value in the reported unit (ug/g dry weight).",
    ...
    "PMX_UNT_CODE",                "unit",                        "Unit of measurement: ug/g dry weight (all rows). Note: raw encoding may show as '\\xb5g/g dw' due to the micro character.",
  ...
  # ── Read the whole export (23,542 rows) ────────────────────────────────────
  # The raw file mis-encodes the micro sign; normalise it to the same UTF-8 form
  # the rest of the pipeline uses.
  all_data <- read_csv(data_file, show_col_types = FALSE) |>
    rename(all_of(stats::setNames(col_rename$old, col_rename$new))) |>
    mutate(unit = ifelse(unit == "\xb5g/g dw", "\u00b5g/g dw", unit))

  # ── project: one row per project_survey_id ─────────────────────────────────
  df_project <- all_data |>
  ...
  # ── station: deduplicate on station_code + lat/lon, surrogate key ──────────
  df_station <- all_data |>
  ...
  # ── parameter: classify as metal or PCB ────────────────────────────────────
  ...
  if (verbose) {
    cat(sprintf("4demon parsed: project %d, station %d, parameter %d, method %d, sample %d, sediment %d\n",
                nrow(df_project), nrow(df_station), nrow(df_parameter),
                nrow(df_method), nrow(df_sample), nrow(df_sediment)))
  }

  list(project   = df_project,
       station   = df_station,
