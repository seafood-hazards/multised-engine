# ── Slim step 1, ICES-DOME ───────────────────────────────────────────────────
# Pilot tables: code_lookup, project, site, parameter, lld, analysis_method,
# reference, sample, sediment.
#
# The natural (unkeyed) joins below are kept exactly as the original script had
# them: several resolve against a `code_lookup` slice whose key is not obvious
# from the call site, and spelling them out risks a silent change of meaning.

slim_transform_ices_dome <- function(con_src) {
  # ── 0. Read source data ────────────────────────────────────────────────────
