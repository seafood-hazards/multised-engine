...
df_mariano_cruise <- df_cruise |>
  dplyr::select(cruise_id, type, year)
...
df_mariano_sample <- df_sample |>
  dplyr::select(cruise_id, core_id, sample_id, depth_from, depth_to)
...
df_mariano_sediment <- df_sediment |>
  ...
  dplyr::select(cruise_id, core_id, sample_id, type, year, core_name, sampling_tool,
